...
def account_request_approve(request, pk):
    ...
    try:
        ...
        if(account.role == "PH"):
            Physician.objects.create(account_id=account.id)
        elif(account.role == "PA"):
            Patient.objects.create(account_id=account.id)
...
def all_doctors_page(request):
    ...
    doctors = []
    for p in physicians:
        doctors.append([Account.objects.get(id=p.account_id), p.specializations, p.hospital affiliations])
    #print(physicians)
    #print(doctors)
