...
def account_request_approve(request, pk):
    ...
    try:
        ...
        #add blank entry in physicians/patients as needed
            if(account.role == "PH"):
                Physician.objects.create(account_id=instance.id)
            elif(account.role == "PA"):
                Patient.objects.create(account_id=instance.id)
...
def all_doctors_page(request):
    ...
    doctors = []
    for p in physicians:
        doctors.append(Account.objects.filter(id=p.account_id))
    #doctors = Account.objects.filter(role="PH")
    print(doctors)
...
def all_patients_page(request):
    ...
    #patients = Account.objects.filter(role="PA")
